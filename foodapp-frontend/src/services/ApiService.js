...
export default class ApiService {
  ...
  static async getAllUsers() {
    const resp = await axios.get(`${this.BASE_URL}/admin/users`, {
  ...
  static async getUserById(id) {
    const resp = await axios.get(`${this.BASE_URL}/admin/users/${id}`, {
      headers: this.getHeader(),
    });
    return resp.data;
  }
  ...
  static async getAllPayments(paymentStatus, page = 0, size = 200) {
    let url = `${this.BASE_URL}/payments/all`;

    if (paymentStatus) {
      url = `${this.BASE_URL}/payments/all?paymentStatus=${paymentStatus}&page=${page}&size=${size}`;
    }
